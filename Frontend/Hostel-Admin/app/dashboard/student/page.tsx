...
import { BedDouble, Bell, Calendar, CreditCard, Info, MessageSquare, Utensils, Clock, AlertTriangle, Loader2, CheckCircle2, XCircle, User, Home, ShieldAlert, Star, ChevronDown } from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch student data
        const studentRes = await axios.get(`${API_BASE_URL}/student/profile`, {
  ...
  const getCategoryBadge = (category: string) => {
    ...
    return (
      <Badge className={`${variant.bg} ${variant.text} capitalize`}>
  ...
  const openFeedbackDialog = (menuItem: MenuItem) => {
    ...
    const existingReview = menuItem.reviews.find(review => 
      review.userId.toString() === (studentData?.id || '').toString()
    );
    if (existingReview) {
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      {urgentNotices.length > 0 && (
        <div className="space-y-2">
          {urgentNotices.map((notice) => (
            <Alert key={notice._id} className="bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-900/20 border-red-200 dark:border-red-800 shadow-sm">
              ...
              <>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <Badge variant="secondary" className="px-2 py-1 text-xs">
                    ...
                    {studentData?.roommates} Roommates
                  </Badge>
        ...
        <Card className="border-indigo-100 dark:border-indigo-900/50 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
          ...
          <CardContent className="relative z-10">
            {isLoading.student ? (
              ...
              <>
                <div className="text-2xl font-bold text-gray-900 dark:text-white">
                  ₹{studentData?.feeDue?.toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Due by {new Date(studentData?.feeDueDate || "").toLocaleDateString()}
                </p>
                <div className="mt-4">
                  <Button size="sm" className="h-8 w-full bg-red-600 hover:bg-red-700" asChild>
      ...
      <Tabs defaultValue="notices" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-indigo-900/10 dark:to-indigo-900/20 p-1">
          <TabsTrigger value="notices" className="data-[state=active]:bg-white dark:data-[state=active]:bg-gray-800 data-[state=active]:shadow-sm">
            <Bell className="h-4 w-4 mr-2" />
            Notices
          </TabsTrigger>
          <TabsTrigger value="mess" className="data-[state=active]:bg-white dark:data-[state=active]:bg-gray-800 data-[state=active]:shadow-sm">
            <Utensils className="h-4 w-4 mr-2" />
            Mess Menu
          </TabsTrigger>
          <TabsTrigger value="activities" className="data-[state=active]:bg-white dark:data-[state=active]:bg-gray-800 data-[state=active]:shadow-sm">
          ...
          <div className="divide-y">
            {isLoading.notices ? (
              ...
              notices.slice(0, 5).map((notice) => (
                <div key={notice._id} className="p-4 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-colors">
                  <div className="flex gap-3 items-start">
                    <div className={`p-2 rounded-lg mt-0.5 ${
                      notice.importance === 'high' ? 'bg-red-100 dark:bg-red-900/20' : 
                      notice.importance === 'medium' ? 'bg-amber-100 dark:bg-amber-900/20' : 
                      'bg-indigo-100 dark:bg-indigo-900/20'
                    }`}>
                      <Bell className={`h-4 w-4 ${
                        notice.importance === 'high' ? 'text-red-600 dark:text-red-400' : 
                        notice.importance === 'medium' ? 'text-amber-600 dark:text-amber-400' : 
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm font-medium">
                        {todayMenuItem.averageRating?.toFixed(1) || '0.0'}
                      </span>
                    </div>
                  </div>
                      ...
                      <Card key={`today-${meal.key}`} className="border-blue-100 dark:border-blue-900/30">
                        ...
                        <CardContent className="p-3 pt-0">
                          <p className="text-sm whitespace-pre-line">
                            {todayMenuItem[meal.key as keyof MenuItem]}
                          </p>
                        </CardContent>
                      </Card>
                    ))}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <ChevronDown 
                              className={`h-4 w-4 transition-transform ${
                                expandedDay === item._id ? 'rotate-180' : ''
                              }`}
                            />
                          </div>
                        </div>
                ...
                return activities.length === 0 ? (
                  ...
                  <div className="divide-y">
                    {activities.slice(0, 5).map((activity) => (
                      <div key={`${activity.type}-${activity.id}`} className="p-4 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-colors">
                        <div className="flex gap-3 items-start">
                          <div className={`p-2 rounded-full ${
                            activity.type === 'complaint' ? 'bg-amber-100 dark:bg-amber-900/20' : 
                            activity.type === 'leave' ? 'bg-purple-100 dark:bg-purple-900/20' : 
                            'bg-indigo-100 dark:bg-indigo-900/20'
                          }`}>
                            {activity.type === 'complaint' && <MessageSquare className="h-5 w-5 text-amber-600 dark:text-amber-400" />}
                            {activity.type === 'leave' && <Calendar className="h-5 w-5 text-purple-600 dark:text-purple-400" />}
                            {activity.type === 'notice' && <Bell className="h-5 w-5 text-indigo-600 dark:text-indigo-400" />}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              ...
                              {activity.type === 'leave' && (
                                <Badge variant="outline" className="text-xs">
                                  {activity.type === 'leave' && 'startDate' in activity && 'endDate' in activity && (
                                    `${format(activity.startDate, "MMM d")} - ${format(activity.endDate, "MMM d, yyyy")}`
                                  )}
                                </Badge>
                              )}
                              {activity.type === 'complaint' && 'category' in activity && getCategoryBadge(activity.category)}
                              {activity.type === 'notice' && 'category' in activity && activity.category && getCategoryBadge(activity.category)}
